...
def experiment(
    ...
    names, agents = create_Critics(methods, agents)
    ...
    for j, name in enumerate(names):
        ...
        for i, N in enumerate(tqdm(results["N"])):
            ...
            results.loc[results.index[i], name] = run_all_attempts(env, agent, N)

    print(results)
    plot_results(results)

def run_all_attempts(env, agent, N):

    sum_steps_to_learn = 0

    for _ in trange(0, attempts_per_N):

        steps_to_learn = agent.method(env, agent.critic, episodes=1000, verbose=False)
        sum_steps_to_learn += steps_to_learn

    average_steps_to_learn = sum_steps_to_learn/attempts_per_chain_length
...
def create_critics(methods, critics):
    ...
    for critic_name, critic in critics.items():
        for method_name, method in methods.items():
            names.append(method_name + " " + agent_name)
...
def plot_results(results):
    ...
    results_melted = pd.melt(results, id_vars=["N"])
