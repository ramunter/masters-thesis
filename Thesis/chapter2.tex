...
Upon reaching the goal the game is over, making the goal state a \textit{terminal state}. Environments with terminal states are called \textit{episodic} as during training the game must be reset to keep playing. If instead the player is teleported back to the start and can keep playing it is called \textit{continuous} environment.
...
The termination condition is given by equation \ref{eq:policy_eval_termination_condition} as in policy iteration. When the value function has converged a policy is given by
...
\begin{equation}
    ...
    Q_\pi(S_t, A_t) = Q_\pi(S_t, A_t) + \alpha\big[R_{t+1} + \gamma \max_a Q_\pi(S_{t+1}, a) - Q_\pi(S_t, A_t)]
\end{equation}
...
Ideally the value estimate method should be able to generalize to states with limited experiences or completely unseen states. In the stock price example the value and policy at similar prices should give information about how the agent should act given an unseen price. 
...
Instead the common approach is to apply stochastic gradient descent over the parameters \citep[p~.201-203]{sutton_barto_2018}. With a monte carlo target this gives the equation 
...
One popular class of nonlinear models are neural networks. This class covers a large variety of models. The simplest and perhaps most used model is the fully connected neural network(NN). A fully connected NN consists of sets of neurons, called layers, where each neuron receives an input from every neuron in the previous layer.

A neuron is simply a regression model where the output is passed through a function named the activation function. This function is usually non-linear to allow the NN to model non-linear functions. Mathematically a neuron is expressed as 
...
Certain methods in this thesis will also use convolutional neural networks (CNN) in combination with fully connected NNs. This is not vital to the understanding of the work done in this thesis so only a brief introduction of CNNs will be provided. The following is summarized from chapter 9 in \cite{goodfellow_2013}.

Images tend to be high dimensional objects meaning they generally consist of a large set of numbers. Running a fully connected NN that maps from this large input to some final output requires a lot of parameters and is computationally heavy. In addition a fully connected NN does not share any of its parameters. This means that if the network has learned a feature in one part of the image it will still not recognize the same feature in another part of the image.

CNNs are a different NN structure designed towards use cases such as images.  A convolution operator is essentially a weighted sum of some input. The weighting scheme is called the kernel. Performing a convolution on a 2D datastructure can be seen in figure \ref{fig:conv}.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth, keepaspectratio]{conv_kernel.png}
    \captionsetup{width=.7\linewidth}
    \caption{\textbf{Convolutional Operator on a 2D object}. The illustration is from \cite{goodfellow_2013}}
    \label{fig:conv}
\end{figure}

The weights in a CNN are the weights of the kernel. The learned convolutional operator is then applied to the entire image as seen in equation \ref{eq:convolution}.
...
where $I$ is the input image, $K$ is the kernel and $(m,n)$ is the size of the kernel. Usually a CNN layer consists of many kernels which learn different features. Since the same weights are used across the entire image the method is translation invariant, meaning a CNN  can recognize features regardless of their placement.
...
The optimal policy is to always move right. However an agent following the policy in equation \ref{eq:ucb} will quickly end up underestimating the value of states to the right as multiple visits to $S_2$ would lead to a low exploration bonus despite the fact that states further to the right have not been properly explored. If this occurs before the agent reaches $S_N$ it will never find the higher reward to the right and end up with a suboptimal policy.
