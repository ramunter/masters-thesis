...
Due to the results of \cite{carlos_2018} this thesis attempts to combine neural networks and the BNIG model through a neural linear setup. To start off a summary of the archicture used in \cite{azziz_2018} is provided. This is used as a base which will modified to fit with the BNIG model and thus allow for better variance propagation.

The BDQN architecture consists of a BN model combined with the classic DQN neural network architecture\citep{mnih_2015}. The final layer of a DQN is a linear layer which means to combine the models one can simply replace the final layer with a BN model. However training the model now needs to be split into two processes. 
