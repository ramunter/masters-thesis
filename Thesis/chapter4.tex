...
\begin{algorithm}[H]
    ...
    \For{episode = 1, M}{
        ...
        \For{t=1, T}{
            $a_t = \argmax_a \phi(s_t)\beta_a + \varepsilon_a$ \\
...
\begin{algorithm}[H]
    ...
    \label{alg:bdqn_train}
        \If{$t \; \textbf{mod} \; T_{train}$ == $0$}{
            ...
            $a' = \argmax_a \phi(s_t)\beta_a + \varepsilon_a$ for each transition \\
            ...
            \end{cases}$ \\
            One step using optimizer of choice on the Huber loss (equation \ref{eq:huber_loss}) on the network weights.\\
