...
\begin{algorithm}[H]
    ...
    \For{episode = 1, M}{
        ...
        \For{t=1, T}{
            ...
            Train online network and model using algorithm \ref{alg:bdqn_train}\\
...
Here $\varepsilon_a$ represents a sample from $N(0,\sigma^2_a)$ and $T_{sync}$ is a hyperparameter that defines how often to sync the online and target network and model. The initialization process requires
...
and finally the training procedure is defined in algorithm \ref{alg:bdqn_train}.
    ...
    \label{alg:bdqn_train}
        \If{$t \; \textbf{mod} \; T_{train}$ == $0$}{
            ...
            Set $y_j = \begin{cases}
                ...
                r_j + \gamma\bigg( \phi(s_t, a')\beta_{a'} + \varepsilon_{a'}\bigg)\quad \text{else}
            \end{cases}$ \\
...
The environment implementation from OpenAI Gym was used \citep{OpenAI_gym}. Each method was run for 100,000 steps with 10 different seeds and every 2000 steps an evaluation phase is run over 2000 steps. The results are summarized in \ref{fig:nn_acrobot}
...
\todo Currently running new run since DQN was wrong
